...
public class ProductPageSteps {
    ...
    public void clickOnProduct(boolean isOnSale, boolean hasRating) {
        ...
        if (isOnSale == true && hasRating == true) {
            searchPage.filterByNavigationGroup("Promotion");
            searchPage.filterByNavigationGroup("Customer Rating");
...
//facebook click
    ...
    public void inputQuantityOfProduct(CharSequence qty) {
        ...
        productDetailsPage.quantity.sendKeys(qty);
    }

    @Step
    public void shouldNotSeeFreeShippingMessageForPriceProductUnder$75InAddedToYourCartDialog() {
        browser.waitFor(getDriver().findElement(addedToCartDialog));
        Assert.assertFalse(browser.isElementPresent(productDetailsPage.shippingPromotion));
    }

    @Step
    public void shouldSeeFreeShippingMessageForPriceProductOver$75InAddedToYourCartDialog() {
        browser.waitFor(getDriver().findElement(addedToCartDialog));
        Assert.assertTrue(browser.isElementPresent(productDetailsPage.shippingPromotion));
        assertEquals("This item ships FREE", productDetailsPage.shippingPromotion.getText());
    }

    @Step public void closeAddToYourCarDialog() {
        getDriver().findElement(closeCartDialogButton).click();
    }

    @Step public void selectValueOfAdditionalOption(String value) {

        JavascriptExecutor executor = (JavascriptExecutor)getDriver();
        executor.executeScript("$('.displayValue').filter(function(){return $.trim($(this).text())==='"+value+"';}).parent().find('.swatchanchor').click()");
    }

    @Step public void shouldSeePriceInSinglePriceFormat() {
        String price = productDetailsPage.price.getText();
        Assert.assertTrue(price.matches("^\\$\\d+\\.\\d{2}$"));
    }

    @Step public void shouldSeePriceInRangePriceFormat() {
        String price = productDetailsPage.rangePrice.getText();
        Assert.assertTrue(price.matches("^\\$\\d+\\.\\d{2}\\ - \\$\\d+\\.\\d{2}$"));
    }

    @Step public void shouldSeeSalePriceInRangePriceFormat() {
        String generalPrice = productDetailsPage.rangePrice.getText();
        prices = generalPrice.split("\n");
        Assert.assertTrue(prices[0].matches("^Sale:.+\\$\\d+\\.\\d{2}\\ - \\$\\d+\\.\\d{2}$"));
    }

    @Step public void shouldSeeRetailPriceInRangePriceFormat() {
        Assert.assertTrue(prices[1].matches("^\\$\\d+\\.\\d{2}\\ - \\$\\d+\\.\\d{2}$"));
    }

    @Step public void shouldSeeNonCouponPromotionDiscount() {
        Assert.assertTrue(prices[2].matches("^You Save: \\d+%$"));
    }

    @Step public void shouldSeeSalePriceInSinglePriceFormat() {
        String generalPrice = productDetailsPage.rangePrice.getText();
        prices = generalPrice.split("\n");
        Assert.assertTrue(prices[0].matches("^Sale:.+\\$\\d+\\.\\d{2}$"));
    }

    @Step public void shouldSeeRetailPriceInSinglePriceFormat() {
        Assert.assertTrue(prices[1].matches("^\\$\\d+\\.\\d{2}$"));
    }
